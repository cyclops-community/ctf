...
/*      remap_tensor(stype->tid_C, C, C->topo, old_size_C,
  ...
  void contraction::prepare() {
    int stat;
    CommData global_comm = C->wrld->cdt;
    int rank = C->wrld->rank;
    ctr * ctrf;
    ...
    if (A == B || A == C){
      // TODO: need to make a copy of A, and set new_ctr.A to the new tensor
      // new_ctr.contract()
      IASSERT(0);
    }
    if (B == C){
      // TODO: need to make a copy of B, and set new_ctr.B to the new tensor
      // new_ctr.contract()
      IASSERT(0);
    }
    if (B->is_sparse && !A->is_sparse){
    ...
    A->was_home = A->is_home;
    B->was_home = B->is_home;
    C->was_home = C->is_home;
    
    mapping *home_map_A = new mapping[A->order];
    copy_mapping(A->order, A->edge_map, home_map_A);
    mapping *home_map_B = new mapping[B->order];
    copy_mapping(B->order, B->edge_map, home_map_B);
    mapping *home_map_C = new mapping[C->order];
    copy_mapping(C->order, C->edge_map, home_map_C);

    topology *home_topo_A = A->topo;
    topology *home_topo_B = B->topo;
    topology *home_topo_C = C->topo;
    ...
    if (A->was_home && !A->is_home) {
      ...
      if (A->is_sparse){
        A->sr->pair_dealloc(A->home_buffer);
      }
      else {
        A->sr->dealloc(A->home_buffer);
      }
    }
    else {
      delete [] home_map_A;
