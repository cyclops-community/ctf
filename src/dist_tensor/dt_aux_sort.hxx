...
/*  CTF_alloc_ptr(ndim*sizeof(int), (void**)&inv_edge_len);
  ...
  memset(bucket_counts, 0, sizeof(long_int)*np); 
#ifdef USE_OMP
  long_int * sub_counts, * sub_offs;
  CTF_alloc_ptr(np*sizeof(long_int)*omp_get_max_threads(), (void**)&sub_counts);
  CTF_alloc_ptr(np*sizeof(long_int)*omp_get_max_threads(), (void**)&sub_offs);
  memset(sub_counts, 0, np*sizeof(long_int)*omp_get_max_threads());
#endif
...
/*    for (j=0; j<ndim; j++){
  ...
  /* reset counts */
#ifdef USE_OMP
  memset(sub_offs, 0, sizeof(long_int)*np);
...
void bucket_by_virt(int const               ndim,
                    ...
                    tkv_pair<dtype> *       bucket_data){
  long_int i, j, loc;
  long_int * virt_counts, * virt_prefix, * virt_lda;
  key k;
  TAU_FSTART(bucket_by_virt);
  
  CTF_alloc_ptr(num_virt*sizeof(long_int), (void**)&virt_counts);
  CTF_alloc_ptr(num_virt*sizeof(long_int), (void**)&virt_prefix);
  CTF_alloc_ptr(ndim*sizeof(long_int), (void**)&virt_lda);
  ...
  memset(virt_counts, 0, sizeof(long_int)*num_virt); 
#ifdef USE_OMP
  long_int * sub_counts, * sub_offs;
  CTF_alloc_ptr(num_virt*sizeof(long_int)*omp_get_max_threads(), (void**)&sub_counts);
  CTF_alloc_ptr(num_virt*sizeof(long_int)*omp_get_max_threads(), (void**)&sub_offs);
  memset(sub_counts, 0, num_virt*sizeof(long_int)*omp_get_max_threads());
#endif
  ...
  memset(sub_offs, 0, sizeof(long_int)*num_virt);
  for (i=1; i<omp_get_max_threads(); i++){
