...
namespace CTF_int {
  ...
  CTF_alloc_ptr(pad_el*(sizeof(int64_t)+sr.el_size), (void**)&padded_pairs);
  new_el = 0;
  offset = 0;
  outside = -1;
  virt_lda=1;
    ...
    for (;;){
      ...
      if (outside == -1){
        for (i=0; i<pad_max-imax; i++){
          ...
          padded_pairs[new_el+i].d = sr.addid;
        }
        new_el+=pad_max-imax;
      }  else {
        for (i=0; i<pad_max; i++){
  ...
  memcpy(padded_pairs+new_el, old_data,  size*(sizeof(int64_t)+sr.el_size));
