...
//    alloc_ptr(nnz*sr->pair_size(), (void**)&buf);
...
//      printf("rank = %d, in key = %ld, new_key = %ld, val = %lf\n",  phys_rank[0], save_key, new_key, ((double*)pi_new[i].d())[0]);
    }
    ...
    if (check_padding){
      ...
      old_nnz_blk_B = (int64_t*)alloc(sizeof(int64_t)*nvirt_B);
      memcpy(old_nnz_blk_B, nnz_blk, sizeof(int64_t)*nvirt_B);
      memset(nnz_blk, 0, sizeof(int64_t)*nvirt_B);
    }
