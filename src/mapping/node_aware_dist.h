...
namespace CTF_int {
   ...
   * \return vector of inter node processor grids of total size equal to the number of ndoes and of same dimension as rGrid, where each dimension divides into the respective dimension of rGrid
   */
  std::vector< std::vector<int> *> get_inter_node_grids(const int * rGrid, int nodes);
}
