...
#undef NORM_INFTY_INST
  ...
  void Tensor<dtype>::TTTP(int num_ops, int const * modes, Tensor<dtype> ** mat_list, bool aux_mode_first){
    ...
    int * phys_phase = this->calc_phys_phase();
    ...
    if (is_vec){
      ...
      do {
        int kd = (k+div-1)/div;
        for (int i=0; i<num_ops; i++){
          tot_size += mat_list[i]->lens[aux_mode_first]*kd/phys_phase[mode[i]];
        }
        if (div > 1)
          tot_size += npair;
        if (tot_size*sizeof(dtype) > max_memuse){
    ...
    MPI_Allreduce(MPI_IN_PLACE, &div, 1, MPI_INT, MPI_MAX, this->wrld->comm);
    dtype * acc_arr = NULL;
    if (!is_vec && div>1){
      //for (int i=0; i<num_ops; i++){
      //  int64_t size;
      ...
      //}
      //} else {
      acc_arr = (dtype*)this->sr->alloc(npair);
      ...
      for (int i=0; i<num_ops; i++){
        ...
        if (phys_phase[modes[i]] == 1 && div==1){
          ...
          if (phys_phse[modes[i]] == 1){
            ...
            Matrix<dtype> mat = mat_list[i]->slice(slice_st, slice_ends);
//            redist_mats[i] = new Matrix<dtype>(nrow, ncol, *this->wrld, *this->sr);
//            redist_mats[i]->slice(zeros,redist_mats[i]->lens, 0.0, *mat_list[i], slice_st, slice_ends, 1.0);
            arrs[i] = (dtype*)this->sr->alloc(nrow*ncol);
            mat.read_all(arrs[i], true);
            redist_mats[i] = NULL;
          } else {
            int nrow, ncol;
            int topo_dim = this->edge_map[modes[i]].cdt;
            IASSERT(this->edge_map[modes[i]].type == PHYSICAL_MAP);
            IASSERT(!this->edge_map[modes[i]].has_child || this->edge_map[modes[i]].child->type != PHYSICAL_MAP);
            if (is_vec){
              Vector<dtype> * v = new Vector<dtype>(mat_list[i]->lens[0], par_idx[topo_dim], par[par_idx], 0, this->wrld, this->sr);
              ...
              if (aux_mode_first){
                nrow = kd;
                ncol = this->lens[mode[i]];
                mat_idx[0] = 'a';
                mat_idx[1] = par_idx[topo_dim];
              } else {
                nrow = this->lens[mode[i]];
                ncol = kd;
                mat_idx[0] = par_idx[topo_dim];
                mat_idx[1] = 'a';
              }
              Matrix<dtype> * m = new Matrix<dtype>(nrow, ncol, mat_idx, par[par_idx], 0, this->wrld, this->sr);
